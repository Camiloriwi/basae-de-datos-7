...
SELECT DISTINCT modelo,placa,color,marca,tipo_vehiculo FROM vehiculos INNER JOIN colores INNER JOIN tipo_vehiculos INNER JOIN marcas WHERE vehiculos.`ID`= 1  AND colores.`ID`= 2 AND marcas.`ID`= 2 AND tipo_vehiculos.`ID`=1;
